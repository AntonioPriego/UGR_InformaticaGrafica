...
typedef enum {DESACTIVADO, MATERIAL, SUAVE, PLANA} tipoIluminacion;
...
class Malla3D
{
 public:
	...
	void draw_Iluminado_Inmediato();
	// Visualización Diferido
	void draw_Iluminado_Diferido();
	...
	//void setTextura(std::string &archivo="text-madera.jpg");


 protected:
 	bool material_activo=false;
	bool calcular_normales()   ; // calcula tabla de normales de vértices (práctica 3)
	bool calcular_coorTextura(bool mosaico=true); // calcula las coordenadas de la textura paramétricas
	bool calcular_coorTexturaCilindrica(); // calcula las coordenadas de la textura cilíndricas
	...
	std::vector<Tupla3f> cA;	// una terna de 3 enteros por cada color de vertice modoAjedrez (1)
	...
	bool coordtex_calculadas=false; // Si se han calculado las normales (se ha dibujado al menos una vez)
