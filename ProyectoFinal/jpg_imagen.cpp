...
Imagen::Imagen( const std::string & nombreArchivo ) 
{
	...
	string nombreCompletoArchivo = "texturas/";
	nombreCompletoArchivo += nombreArchivo + ".jpg";
	...
	buf = JpegFile::JpegFileToRGB( nombreCompletoArchivo.c_str(), &w, &h ) ;
...
void Imagen::escribirEn( const std::string & nombreArchivo )
{
    ...
    **/
	...
	bool result = bool( JpegFile::RGBToJpegFile( nombreCompletoArchivo.c_str(), buf,w,h,1,100) );
